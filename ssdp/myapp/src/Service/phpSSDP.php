...
class phpSSDP {
    ...
    private static function _search($st, $timeout, $mx = null) {
        ...
        $send_ret = socket_sendto($socket, $headers, strlen($headers), 0, '239.255.255.250', 1900);
    ...
    public static function getDevicesByURN($urn, $json = false, $timeout = 1, $mx = null) {
        ...
        foreach ($devices as $key => $device) {
            $devices[$key]['DESCRIPTION'] = self::_getDeviceInfo($device['LOCATION']);
        }
         return self::_sendResponse(self::_sortByIP($devices), $json);
        }
        return NULL;
...
require_once './phpSSDP.php';


$jose=new \LqdT\phpSSDP();

//$array=$jose->getAllRootDevices();
$array=$jose->getDevicesByURN('urn:schemas-upnp-org:service:ContentDirectory:1');
print_r($array);
*/
